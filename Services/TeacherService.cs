...
namespace school_management_backend.Services
{
    public class TeacherService : ITeacherService
    {
        ...
        public TeacherService(DBContext context, IConfiguration config)
        {
            _context = context;
            _config = config;
        }

        public Teacher GetTeacherById(Guid id, string token)
        {
            return _context.Teachers.Include(e => e.Token).FirstOrDefault(usr => usr.Id == id);
        }
        public Teacher GetTeacherByUserName(string userName)
        {
            var teacher = _context.Teachers.Where(x => x.UserName == userName)
                .Include(x => x.School).Where(x => x.SchoolId == x.School.Id)
                .FirstOrDefault();
            return teacher;
        }
        public string generateJwtToken(Teacher user)
        {
        ...
        public void AddToken(string token, Teacher user)
        {
        ...
        public dynamic ReturnResult(string token, Teacher user)
        {
